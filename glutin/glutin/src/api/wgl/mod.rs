...
unsafe fn choose_arb_pixel_format_id(
    ...
    let mut format_id = std::mem::MaybeUninit::uninit();
    let mut num_formats = std::mem::MaybeUninit::uninit();
    if extra.ChoosePixelFormatARB(
        hdc as *const _,
        descriptor.as_ptr(),
        std::ptr::null(),
        1,
        &mut *format_id.as_mut_ptr(),
        &mut *num_formats.as_mut_ptr(),
    ) == 0
    {
        return Err(());
    }

    if num_formats.assume_init() == 0 {
        return Err(());
    }

    Ok(format_id.assume_init())
}
...
fn choose_dummy_pixel_format(hdc: HDC) -> Result<raw::c_int, CreationError> {
    ...
    if pf_id == 0 {
        return Err(CreationError::OsError(
            "No available pixel format".to_owned(),
        ));
    }
